...
open class CloudServicesControllerIntegrationTests {
    ...
    fun testTokenGeneration(){
        ...
        val testValue = restTemplate.getForEntity("$protocol://localhost:$port/auth", AuthenticationToken::class.java)
        assert(testValue.statusCode==HttpStatus.OK)
        assert(testValue.hasBody())
        //test with token generated by username/password login
        if (useSSL!=null && useSSL!="") {
            protocol= "https"
            trustSelfSignedSSL()
        ...
        val testValue2 = restTemplate.exchange("$protocol://localhost:$port/auth",HttpMethod.GET, httpEntity, AuthenticationToken::class.java)
